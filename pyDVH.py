...
#		DoseList=[]
#		VolList=[]
#		for ii, line in list(enumerate(fin)):
#			values=line.split()
#			if len(values)==3:
#				#print "ii: "+str(ii)+" val: "+str(values[0])
#				if ii>20 and float(values[0])==0.0:
#					#print "break crit true"
#					break
#				else:
#					DoseList.append(float(values[0]))
#					VolList.append(float(values[1]))
#			else:
#				continue
#		#print DoseList
#		#print VolList
#		
#		# from right side
#		for jj, el in reversed(list(enumerate(VolList))):
#			if el >= n:
#				#D_right.append( [ DoseList[jj], el ] )
#				LeftList=[ DoseList[jj], el ]
#				break
#		#print LeftList
#		
#		#from left side
#		for jj, el in list(enumerate(VolList)):
#			if el <= n:
#				#D_left.append( [ DoseList[jj], el ] )
#				RightList=[ DoseList[jj], el ]
#				break
#		#print RightList
#		
#		slope = (RightList[1] - LeftList[1])/(RightList[0] - LeftList[0])
#		#print slope
#		offset = LeftList[1] - slope * LeftList[0]
#		#print offset
#		finalvalue= (n - offset) / slope
#		#print finalvalue
#		
#	return result
#
#def OARDoseVolumeLimit(OAR, path):
#	global constraints
#	state='searching header'
#	if OAR in constraints.keys():
#		dose_max=constraints[OAR]['dose_max']
#		with open(path,'r') as fin:
#			#print fin
#			lines=fin.readlines()
#			#print len(lines)
...
#				return 0
#			for line in lines:
#				if state=='searching header':
#					if line.startswith("c: "+ OAR):
#						volume=float(line.split()[7])
#						percentage=constraints[OAR]['volume']/volume*1000*100
#						state='ignore line'
#				elif state=='ignore line':
#					state='searching percentage'
#				elif state=='searching percentage':
#					values=line.split()
#					if len(values)!=3:
#						raise Exception('something is wrong!')
#					values[0]=int(values[0])
#					values[1]=float(values[1])      
#					if values[1]<=percentage:
#						#print str(values[1])
#						relevantPercentage=values[1]
#						if relevantPercentage == 0: 
#							return 0
#						sum_dose=0
#						state='calculate average'
#				if state=='calculate average':
#					values=line.split()
#					if len(values)!=3:
#						break
#					else:
...
#						sum_dose+=values[0]*values[2]
#			average_dose=sum_dose/relevantPercentage*dose_max/100.0
#			result=average_dose		
#		return result
#	else: 
#		return 0
...
#		for line in lines:
#			#print "test"
#			if re.match("c:[ ]*{}".format(OAR), line):
#				maxdose=float(line.split()[3])
#		result=maxdose
#	return result
#
#def OARMeanDose(OAR,path):
#	try:
#		with open(path,'r') as fin:
#			lines=fin.readlines()
#			if len(lines) < 130:
...
#			result=meandose
#	except:
#		print("Error in File: '{}' OAR: '{}'".format(path,OAR))
#		raise
#	return result
#
#pattern_dict={}
#def OARMeanMaxDose(OAR,path):
##	global pattern_dict
##	if not OAR in pattern_dict:
##		pattern_dict[OAR]=re.compile("c:[ ]*{}".format(OAR))
##	pattern=pattern_dict[OAR]
#	try:
#		with open(path,'r') as fin:
#			lines=fin.readlines()
#			if len(lines) < 130:
#				#print "File is very short: '{}' OAR: '{}'".format(path,OAR)
#				return (0,0)
#			for line in lines:
#				#print OAR
#				if re.match("c:[ ]*{}".format(OAR), line):
#					return (float(line.split()[4]),float(line.split()[3]))
#			raise Exception('Cannot find OAR', path)
...
#				continue
#			else:
#				irrvol=float(values[1])
#				break
#		result=irrvol
#	return result
#	
#def getDVHValuesFromFile(filename):
#	"""Read x and y values from GD file for DVH"""
#	with open(filename,'r') as fin:
#		linecounter=[]
#		j=0
#		for line in fin:
...
#				continue
#			if values[0]=="H:":
#				linecounter.append(j)
#		#debug output		
##		print( "lines: ", linecounter[0:len(linecounter)])
#	with open(filename,'r') as fin:
#		xvalues=[]
#		yvalues=[]	
#		jj=0
...
#		for line in fin:
#			jj+=1
#			if len(linecounter)>=2 and jj > linecounter[1]:
#				break
#			values=line.split()
##			if len(values)==0:
##				continue
...
#				yvalues.append(float(0))
#				i=i+1
#	return (xvalues,yvalues)
#      
#def getOARValuesFromFile(filename):
#	"""Read x and y values from GD file for DVH"""
#	with open(filename,'r') as fin:
#		linecounter=[]
#		j=0
...
#			if len(linecounter)>=2 and jj < linecounter[1]:
#				continue			      
##line below critical with gd files with no residual
#			#if len(linecounter)>=2 and jj > linecounter[2]-3:
#				#continue
#			values=line.split()
##			if len(values)==0:
##				continue
...
##
##
##									get extreme values from DVH
##
##
##===============================================================================================#
#def getExtremeDVHValuesFromFileByVOI( filenameList, VOIstr, dTargetDose, dNormDose, bReadDifferential=False ):
#	"""calc average value of DVH lines from same VOIstr but different scenarios"""
#	#normDose=10.0
#	### for FC lungs
#	#normDose=24.0
#	normFactor=dTargetDose/dNormDose
#	#print('dose norm. factor: %f' % normFactor)
#	data=[]
#	yvaluesMaxLen=0
#	xvaluesMaxLen=0
#	xvalues=[]
...
#		if checkVOIStringInDVHfile(filenameList[i],VOIstr):
#			xvalues, yvalues = getDVHValuesFromFileByVOI(filenameList[i],VOIstr, bReadDifferential)
#			yvaluesMaxLen=max(yvaluesMaxLen,len(yvalues))
#			xvaluesMaxLen=max(xvaluesMaxLen,len(xvalues))
#			data.append(yvalues)
#	
#	#yvaluesAverage=[sum(e)/len(e) for e in zip(*data)]
#	yvaluesAverage=[np.mean(e) for e in zip(*data)]
#	yvaluesSTD=[np.std(e) for e in zip(*data)]
#	
#	yvaluesMedian=[np.median(e) for e in zip(*data)]
#	yvaluesQ25=[np.percentile(e,25) for e in zip(*data)]
#	yvaluesQ75=[np.percentile(e,75) for e in zip(*data)]
#	
#	#print yvaluesMedian
#	#print max(yvaluesQ75)
#	#print min(yvaluesQ25)
#	
#	#yvaluesMin = yvaluesQ25
#	#yvaluesMax = yvaluesQ75
#	
#	#print yvaluesAverage
#	#print yvaluesSTD
#	#yvaluesMin = [avg - std for (avg,std) in zip(yvaluesAverage,yvaluesSTD)]
#	#yvaluesMax = [avg + std for (avg,std) in zip(yvaluesAverage,yvaluesSTD)]
#	
#	#for (avg,std) in zip(yvaluesAverage,yvaluesSTD):
#		#low=avg-std
#		#high=avg+std
#		#if low < 0:
#			#low=0
#		#if high > 100:
#			#high=100
#		#yvaluesMin.append(low)
#		#yvaluesMax.append(high)
#		
#		
#	#print yvaluesMin
#	#print yvaluesMax
#	
#	### appending 0.0 to the too short lists
#	for i in range(len(data)):
#		if len(data[i]) < yvaluesMaxLen:
#			#print len(data[i])
#			#print range( len(data[i]),yvaluesMaxLen )
#			for j in range( len(data[i]),yvaluesMaxLen ):
#				data[i].append(0.0)
#	### finding min/max values and write to lists
#	for i in range(yvaluesMaxLen):
#		yvaluesMin.append(min([row[i] for row in data]))
#		yvaluesMax.append(max([row[i] for row in data]))
#		
#	#if len(xvalues) < xvaluesMaxLen:
#		#for i in range( len(xvalues),xvaluesMaxLen ):
#			#xvalues.append(float(i+1))
#			
#			
#	### test: smoothing via running mean
#	averagingWindow=0 # 0 --> identity
#	yvaluesMinSmoothed = runningAverage(yvaluesMin,averagingWindow)
#	yvaluesMaxSmoothed = runningAverage(yvaluesMax,averagingWindow)
#	xvalues = [x / normFactor for x in xvalues]
#	#print yvaluesMin
#	#print yvaluesMinSmoothed
#	return (xvalues, yvaluesMinSmoothed, yvaluesMaxSmoothed)
#	
#	#return (xvalues, yvaluesMin, yvaluesMax)
#
#
#
#
